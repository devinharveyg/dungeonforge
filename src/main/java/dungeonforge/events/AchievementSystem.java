    ...
    public void onEvent(GameEvent event) {
        ...
        switch (event.getType()){
            case MONSTER_DIED -> {
                kills++;
                if (kills == 1){
                    unlock("First Blood", "Defeat your first monster");
                }
                if (kills == 10){
                    unlock("Exterminator", "Defeat 10 monsters");
                }
            }
            case MONSTER_FLED -> {
                fled++;
                if (fled == 1){
                    unlock("Terrifying", "make something run away");
                }
            }
            ...
            case LEVEL_ENTERED -> {
                ...
                if (deepestLevel >= 3){
                    unlock("Deep Delver", "Reach Dungeon level 3");
                }
            }
            case DELVE_SURVIVED -> {
